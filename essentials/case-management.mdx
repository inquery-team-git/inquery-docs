...
The executive case summaries are generated based on the contents of the case. All uploaded files will be considered as input to the case summary by default. In order to filter out specific content from being included in the case summary, you can filter out sections from the case using the standard filter function 
then re-run button the summary using only the content that has not been filtered. You may regenerate or edit the summary as many times as needed.
